...
// Add correctRatio variable near the top with other variables
const correctRatio = 4; // Every group of 4 numboxes will have exactly 1 correct
let currentGroupCorrectIndex = -1; // Track which position in current group of 4 is correct
...
// Add pitch offset for correct sounds (add this near the top with other variables)
let pitchOffset = 0;

// Track the previous correct number for chained equations
...
startBtn.addEventListener("click", () => {
  if (!animationInProgress) {
    ...
    currentGroupCorrectIndex = -1; // Reset the group correct index
    ...
    allShapesToLaunch.forEach((shape, index) => {
      ...
      setTimeout(() => {
        ...
        // Calculate which position we are in the current group of 4
        const positionInGroup = index % correctRatio;

        // If this is the first numbox of a new group, randomly pick which position will be correct
        if (positionInGroup === 0) {
          currentGroupCorrectIndex = Math.floor(Math.random() * correctRatio);
          //console.log(` New group starting - correct numbox will be at position ${currentGroupCorrectIndex + 1} in this group`);
        }

        // Check if this is the randomly selected correct position in the current group
        if (positionInGroup === currentGroupCorrectIndex) {
          numberToDisplay = currentRule;
          //console.log(` Numbox ${index + 1} launching with CORRECT number: ${numberToDisplay} (rule: ${currentRule}) - position ${positionInGroup + 1} in group`);
        } else {
          numberToDisplay = Math.floor(Math.random() * 100); // 0-99
          //console.log(` Numbox ${index + 1} launching with random number: ${numberToDisplay} (rule: ${currentRule}) - position ${positionInGroup + 1} in group`);
        }
